...
def _get_min_score() -> int:
    raw = os.getenv("POLYMARKET_PROTOCOL_MIN_SCORE", "6").strip()
    try:
        return int(raw)
    except Exception:
        return 6
...
def protocol(n=None):
    """
    Reads gated.json from the current directory, which contains BTC-relevant Polymarket events
    ...
    # 过滤掉 importance_score 低于环境变量阈值的事件，并按分数降序排序
    ...
    output = [
        "# Polymarket Prediction Market Data (Gated for BTC Long Hunter)\n",
        f"The following Polymarket events have been filtered for BTC long-horizon relevance (Importance Score >= {MIN_SCORE}).\n"
    ]
