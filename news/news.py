...
def run_task():
    ...
    try:
        # 默认 page=1；这里显式轮询前 100 页并合并
        merged_api_items = []
        seen_urls = set()
        for page in range(1, 101):
            print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Fetching news page {page}/100 ...")
            page_params = dict(params)
            page_params["page"] = page
            with HTTP_SESSION.get(url, params=page_params, timeout=15) as response:
                if response.status_code != 200:
                ...
                page_items = api_data.get("data")
                if not isinstance(page_items, list):
                    print(f"API response format unexpected on page {page}: 'data' key not found or not list.")
                    break
                if not page_items:
                    # 提前结束：后续页通常也为空
                    print(
                        f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] "
                        f"Page {page} returned 0 items, stop pagination."
                    )
                    break
                ...
                for news_item in page_items:
                    ...
                    merged_api_items.append(news_item)
                    page_new += 1

                print(
                    f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] "
                    f"Page {page} fetched {len(page_items)} items, "
                    f"added {page_new} unique, total unique {len(merged_api_items)}."
                )
        ...
        if merged_api_items:
            ...
            new_entries_added = 0
            total_fetched = len(merged_api_items)
            
            # 直接使用 API 返回的数据，不再与本地数据合并（实现原地替换）
            new_news_list = []
            for index, news_item in enumerate(merged_api_items, 1):
                news_url = news_item.get('news_url')
                if not news_url:
                    continue
            ...
            if new_news_list:
                ...
                print(
                    f"Replaced news_data.json with {len(new_news_list)} current articles. "
                    f"(Fetched {new_entries_added} new contents, merged {total_fetched} unique items from <=100 pages)"
                )
            ...
            print("No news data fetched from the first 100 pages.")
    except Exception as e:
